#NVDAExtensionGlobalPlugin\commandKeysSelectiveAnnouncementAndRemanence\specialForGmail.py
...
def manageRemanenceForGmail(vkCode,scanCode,extended,injected):
	...
	if len(keyboardHandler.currentModifiers): return False
	if _trapNextKey:
		_trapNextKey = False
		return False
	gesture = keyboardHandler.KeyboardInputGesture(keyboardHandler.currentModifiers, vkCode, scanCode, extended)
	...
	def stopRemanenceChar():
		...
		_remanenceCharacter   = None
		queueHandler.queueFunction(queueHandler.eventQueue, keyboardHandler.KeyboardInputGesture.fromName(ch).send)
	...
	try:
		...
		ti = None
	if obj.role == controlTypes.ROLE_EDITABLETEXT or controlTypes.STATE_EDITABLE in obj.states or ti is None or not ti.passThrough: return False
	dci = ti.documentConstantIdentifier
	gmail = "https://mail.google.com/mail/"
	if dci[: len(gmail)]  != gmail: return False
	...
	if _remanenceCharacter  is not None:
		...
		_remanenceCharacter  = None
		queueHandler.queueFunction(queueHandler.eventQueue, keyboardHandler.KeyboardInputGesture.fromName(ch0).send)
...
def initialize():
	...
	if not toggleRemanenceForGmailAdvancedOption (False): return
