...
class NVDAExtensionGlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	_scriptsToDocsAndCategory = {
		...
		# Translators: Input help mode message for copy date and time to clipboard command.
...
#		subprocess.call("explorer \"%s\"" %path, shell=True)
	...
	def onCommandKeysSelectiveAnnouncementMenu(self, evt):
		gui.mainFrame._popupSettingsDialog(commandKeysSelectiveAnnouncementAndRemanence.CommandKeysSelectiveAnnouncementDialog)
	...
	def script_DisplayAppModuleInfo(self,gesture):
		...
		if currentAddon is not None:
			...
			info = _("{summary}, version: {version}").format(summary= currentAddon.manifest["summary"], version= currentAddon.manifest["version"])
	...
	def script_nextVoiceProfile(self, gesture):
		from . import switchVoiceProfile
		switchVoiceProfile.SwitchVoiceProfilesManager().nextVoiceProfile(forward = True)
	
	def script_previousVoiceProfile(self, gesture):
		from . import switchVoiceProfile
		switchVoiceProfile.SwitchVoiceProfilesManager().nextVoiceProfile(forward = False)
