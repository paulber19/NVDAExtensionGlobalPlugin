...
def generateObjectSubtreeGetObject(obj,indexGen, th):
	...
	for i in rangeGen(childCount):
		...
		if role ==controlTypes.ROLE_TABLEROW:
			if child.parent.childCount >500 :
				break
			if  invisible:
				continue
		...
		if role == controlTypes.ROLE_TREEVIEWITEM:
			if (child.parent.childCount >500
				or child.parent.role not in [controlTypes.ROLE_TREEVIEW, controlTypes.ROLE_LIST]):
				break
			if  invisible:
				continue
...
def getObjectsHelper_generator(oParent):
	...
	objectList = []
	lastSentIndex=0
	th = runInThread.RepeatBeep(delay = 2.0, beep = (200,200), isRunning= sayAllHandler.isRunning )
	th.start()
...
def findAllNVDAObjects(oParent):
	if ElementListDialog.isRunning():
		...
		return
	sayAllHandler._startGenerator(getObjectsHelper_generator(oParent))
